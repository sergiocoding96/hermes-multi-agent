#!/usr/bin/env bash
# create-research-employee.sh
#
# Create the "research-agent" Paperclip employee backed by the hermes_local
# adapter (built into Paperclip — no separate install needed).
#
# Idempotent: skips creation if an agent using the research-agent profile
# already exists in the company.
#
# Usage:
#   PAPERCLIP_BOARD_TOKEN=<token> ./create-research-employee.sh
#
...
# Explicit CWD avoids FileNotFoundError when the Paperclip server's own CWD is deleted
# (happens when papercliapai was started from a since-removed worktree).
PROFILE="research-agent"
AGENT_NAME="Research Agent"
AGENT_TITLE="Senior Research Analyst"
MODEL="${HERMES_MODEL:-minimax/MiniMax-M2}"
TOOLSETS="terminal,file,web,browser"

log() { printf '[create-research-employee] %s\n' "$*" >&2; }
die() { printf '[create-research-employee] ERROR: %s\n' "$*" >&2; exit 1; }
