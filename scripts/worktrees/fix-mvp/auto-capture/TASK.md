...
- `~/.hermes/plugins/memos-toolset/` — the plugin's source tree (Python or whatever Hermes uses). Find the existing tool-handler entry points first.
...
- New: `memos-toolset/auto_capture.py` — the hook + filter rules + queue.
- New: `memos-toolset/capture_queue.py` — local SQLite retry queue.
- Tests under `memos-toolset/tests/` (mirror existing test patterns; don't introduce a new framework).
