...
// Per-step penalty applied to V when a trace recorded tool errors / failures
// (non-empty error_signatures). These objective outcome signals are captured
// at trace time but were never fed into reward — so a step that hit repeated
// tool failures (e.g. a web_search retry loop) used to score the same as a
// clean step. Subtracting a fixed penalty pushes failed steps below neutral so
// they rank lower and can seed failure_avoidance induction.
const ERROR_OUTCOME_PENALTY = 0.35;
...
export function backprop(input: BackpropInput): BackpropResult {
  ...
  for (let i = input.traces.length - 1; i >= 0; i--) {
    ...
    const Vbase = i === input.traces.length - 1
      ? rHuman // V_T = R_human (V7 §0.6 boundary case)
      : alpha * rHuman + (1 - alpha) * gamma * nextV;

    // Objective outcome penalty: a step that recorded tool errors/failures is
    // pushed below its purely-reflective value so failures rank low and feed
    // failure_avoidance induction.
    const hasError = Array.isArray(t.errorSignatures) && t.errorSignatures.length > 0;
    const V = hasError ? clamp(Vbase - ERROR_OUTCOME_PENALTY, -1, 1) : Vbase;
