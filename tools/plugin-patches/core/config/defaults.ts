...
export const DEFAULT_CONFIG: ResolvedConfig = {
  ...
  algorithm: {
    ...
    skill: {
      // Raised 1 → 3 (2026-05-27). minSupport=1 crystallized a skill from a
      // single episode, producing hyper-specific one-off skills that never
      // re-matched (4 invocations across 41 skills). Requiring the pattern to
      // recur across ≥3 episodes keeps only genuinely reusable procedures.
      minSupport: 3,
      // Raised 0.02 → 0.03: still filters neutral-noise policies (the
      // shrinkage-anchored gain is ∝ V_with−0.5) but a touch stricter now
      // that minSupport demands more evidence.
      minGain: 0.03,
