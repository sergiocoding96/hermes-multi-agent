...
Read the supporting traces' outcomes and utility (each block shows
"V: <value>" — negative V means the step went badly). Classify the policy:

- "success_pattern" — the traces show an action that WORKED. The "action"
  field prescribes what TO DO.
- "failure_avoidance" — the traces show an action that FAILED, looped, or
  produced errors (negative V, error tools, repeated retries). Here the
  policy's job is to capture what NOT to do and the better alternative:
  set "experience_type":"failure_avoidance", keep "action" as the corrective
  template, and fill "anti_patterns" with the specific mistakes to avoid
  ("don't retry web_search after 3 consecutive failures — switch tactic").

Do not force everything to success. If the evidence is failures, say so —
"what to avoid" is as valuable as "what works".
...
Return JSON:
{
  ...
  "experience_type": "success_pattern" | "failure_avoidance",
  "evidence_polarity": "positive" | "negative" | "mixed",
  "anti_patterns": ["specific mistake to avoid", ...],
