...
function renderSkill(c: SkillCandidate, opts: RenderOpts): InjectionSnippet {
  // Attribution tag for cross-agent visibility. Only shown when the skill
  // was crystallized by a DIFFERENT profile than the caller (i.e. it's a
  // shared skill, not the caller's own work). The caller's namespace is
  // not threaded through to the renderer today, so we surface the origin
  // unconditionally — the consumer-side prompt template can decide
  // whether to elide it. Added 2026-05-17.
  const origin = c.ownerProfileId ? ` (learned by ${c.ownerProfileId})` : "";

  if (opts.skillMode === "full") {
    const body = truncate(
      `Skill: ${c.skillName}${origin}\n` + c.invocationGuide.trim(),
    );
    return {
      refKind: "skill",
      refId: c.refId,
