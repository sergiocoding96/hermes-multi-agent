...
export interface InductionDraft {
  ...
  /**
   * Whether this cluster encodes "what worked" (success_pattern) or "what to
   * avoid" (failure_avoidance), classified by the L2 prompt from the supporting
   * traces' outcomes/value. Defaults to success_pattern when absent.
   */
  experienceType?: "success_pattern" | "failure_avoidance";
  /** Evidence polarity of the supporting traces (positive/negative/mixed). */
  evidencePolarity?: "positive" | "negative" | "neutral" | "mixed";
  /** Step-level "do NOT" notes — populated for failure_avoidance policies. */
  antiPatterns?: string[];
}
