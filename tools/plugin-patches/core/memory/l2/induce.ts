...
export function buildPolicyRow(args: {
  ...
  return {
    ...
    // Classified by the L2 prompt from the supporting traces' outcomes so
    // negative/error episodes induce failure_avoidance policies, not just
    // success patterns.
    experienceType: args.draft.experienceType ?? "success_pattern",
    evidencePolarity: args.draft.evidencePolarity ?? "positive",
    // Anti-patterns from the prompt seed the avoid-list at induction time;
    // the decision-repair pipeline still augments it from later feedback.
    decisionGuidance: { preference: [], antiPattern: args.draft.antiPatterns ?? [] },
...
function normaliseDraft(value: Record<string, unknown>, traceIds: readonly TraceId[]): InductionDraft {
  ...
  const experienceType =
    value.experience_type === "failure_avoidance" ? "failure_avoidance" : "success_pattern";
  const evidencePolarity =
    value.evidence_polarity === "negative" ? "negative"
    : value.evidence_polarity === "mixed" ? "mixed"
    : value.evidence_polarity === "neutral" ? "neutral"
    : "positive";
  const antiPatterns = Array.isArray(value.anti_patterns)
    ? sanitizeDerivedMarkdownList((value.anti_patterns as unknown[]).filter((c): c is string => typeof c === "string"))
    : [];
  return {
